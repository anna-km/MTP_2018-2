...
void novo_numero(float * vetor, int tam, float min, float max, LCG * r) 
{
	int i;
	for(i = 0; i < tam; i++)
...
float somar(float *inicio_vetor, float *fim_vetor)
{
	while (inicio_vetor != fim_vetor + 1) 
		return (*inicio_vetor + somar(inicio_vetor + 1, fim_vetor));
	return 0;
}
...
int main()
{
	...
	int a;
	float quantidade[50];
	novo_numero(quantidade, 50, 0.5, 1.5, &random);
	while(a!=3)
	{
		...
		{
			case 1:
				printf ("\nA soma eh %.6f \n\n", somar (&quantidade[0], &quantidade[49]));
				break;
			case 2:
